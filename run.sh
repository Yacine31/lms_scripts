...
bash -x $SCRIPTS_DIR/initdb.sh $PROJECT_NAME 2>/dev/null

# appeler la consolidation des fichiers lms_cpu
bash -x $SCRIPTS_DIR/lms_cpu.sh $CPU_CSV 2>/dev/null

# intégrer les données à la base mysql
echo "import des données serveurs dans MySQL ..."
bash -x $SCRIPTS_DIR/loaddata.sh $CPU_CSV $CPU_TABLE 2>/dev/null

# générer les options de la base et les packs d'admin 
bash -x $SCRIPTS_DIR/db_options.sh $PROJECT_NAME 2>/dev/null

# générer le rapport
bash -x $SCRIPTS_DIR/reports2xml.sh $PROJECT_NAME 2>/dev/null
