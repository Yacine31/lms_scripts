...
find -type f -iname "Collection*zip" -o -iname "Collection*tar" -o -iname "Collection*bz2" -o -iname "Collection*tar.gz" | while read f; do
    case $f in 
        *tar)
            d=$(echo $f | sed 's/LMSCollection-//g' | sed 's/.tar//g');
            export cmd="tar xf $f -C $d 2>&1 1>/dev/null"
        ;;
        *zip)
            d=$(echo $f | sed 's/LMSCollection-//g' | sed 's/.zip//g');
            # unzip -o : ocerwrite without confirmation
            export cmd="unzip -o $f -d $d 2>&1 1>/dev/null"
        ;;
        *bz2)
            d=$(echo $f | sed 's/LMSCollection-//g' | sed 's/.tar.bz2//g');
            export cmd="tar xfj $f -C $d 2>&1 1>/dev/null"
        ;;
        *tar.gz)
            d=$(echo $f | sed 's/LMSCollection-//g' | sed 's/.tar.gz//g');
